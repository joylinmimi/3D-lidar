...
def DBScan():
	...
	#print len(db.components_)
     	print  'core',len(db.core_sample_indices_),db.core_sample_indices_
	#print centroids_
	...
	for k, col in zip(unique_labels, colors):
		...
		array_color.extend(col)
            array_x.extend(xy2[:,0])
            array_y.extend(xy2[:,1])
            array_z.extend(xy2[:,2])
	    #print 'stack array', len(array_x)
            #array_col.extend(col)
	#print array_col
	stack_array.append(array_x)
	stack_array.append(array_y)
	stack_array.append(array_z)
	stack_array.append(array_color)
        #stack_array.append(db.components_)
	#print stack_array
	#stack_array=np.vstack((array_x,array_y,array_z,array_color))
            #print xy2
	return stack_array
	    #print test
	    #print tuple(col)
